...
const Page = () => {
  ...
  useEffect(() => {
    const fetchPriorityTopics = async () => {
      ...
      const priority = topics.filter(topic => topic.priority);
      setPriorityTopics(priority);
    };
  ...
  const handleTogglePriority = (updatedTopic) => {
    setPriorityTopics((prevTopics) =>
      prevTopics.map((t) =>
        t._id === updatedTopic._id ? { ...t, priority: updatedTopic.priority } : t
      )
    );
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      {priorityTopics.length > 0 ? (
        <>
          <h1 className="text-3xl font-bold mb-4">Priority Items</h1>
          {priorityTopics.map((t) => (
            <div
              key={t._id}
              className={`p-4 border border-slate-300 my-3 bg-[#ffffea] rounded-lg ${
        ...
        <p className="text-xl font-bold text-center">No priority items found.</p>
      )}
    </div>
  );
};
